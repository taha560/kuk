...
--vardump(result)
  local user = 'user#id'..result.peer_id
	...
	if is_banned(result.id, extra.chatid) then 
        send_large_msg(chat, 'این فرد بن شده است.')
        send_large_msg(channel, 'این فرد بن شده است.')
	elseif is_gbanned(result.id) then
	    send_large_msg(chat, 'این فرد سوپر بن شده است.')
		send_large_msg(channel, 'این فرد سوپر بن شده است.')
	else    
